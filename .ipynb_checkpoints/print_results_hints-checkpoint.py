...
def print_results(results_dic, results_stats_dic, model, 
    ...
    for key in results_stats_dic:
        # Prints all the percentages in the results_stats_dic dictionary
        if key.startswith('pct'):
            print("{:20}: {:.2f}".format(key, results_stats_dic[key]))

    # Print incorrectly classified dogs if the user requests
    if (print_incorrect_dogs and 
        ((results_stats_dic['n_correct_dogs'] + results_stats_dic['n_correct_notdogs']) != results_stats_dic['n_images'])):
        print("\nINCORRECT Dog/NOT Dog Assignments:")
        
        # Process through results_dic, printing incorrectly classified dogs
    ...
    # Print incorrectly classified breeds if the user requests
    if (print_incorrect_breed and 
        (results_stats_dic['n_correct_dogs'] != results_stats_dic['n_correct_breed'])):
        print("\nINCORRECT Dog Breed Assignment:")
        
        # Process through results_dic, printing incorrectly classified breeds
        for key in results_dic:
